...
array_points_per_test=(0 1.75 4 2.25)
max_array_points=(0 35 40 45)
leak_check=0
...
function check_test() {
	...
	if ([ $problem_no -eq 2 ] && [ $test_no -eq 7 ]) || [ $test_no -eq 17 ]; then
        timeout 70 stdbuf -o0 -e0 valgrind --leak-check=full --error-exitcode=10 ./$exec < $in_path > $out_path 2>/dev/null
	else
		timeout 70 ./$exec < $in_path > $out_path
	fi
